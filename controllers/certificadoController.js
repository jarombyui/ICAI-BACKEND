...
export const emitirCertificado = async (req, res) => {
  try {
    ...
    const plantillaPath = path.join(process.cwd(), 'backend/modelo-certificado/PLANTILLA1__CERTIFICADO.pdf');
    ...
    page.drawText(`Desde: ${inscripcion.fecha_inicio?.toISOString().slice(0,10)}`, { x: 328, y: 230, size: 14, font, color: rgb(0, 0, 0) });
    page.drawText(`Hasta: ${inscripcion.fecha_fin?.toISOString().slice(0,10)}`, { x: 328, y: 210, size: 14, font, color: rgb(0, 0, 0) });
    // Guarda el PDF generado
    const pdfBytes = await pdfDoc.save();
    const outputDir = path.join(process.cwd(), 'backend/certificados');
